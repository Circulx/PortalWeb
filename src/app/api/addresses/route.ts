...
export async function GET(request: Request) {
  try {
    ...
    if (!addresses) {
      return NextResponse.json({ error: "Addresses not found for the user" }, { status: 404 })
    }
...
export async function POST(request: Request) {
  try {
    ...
    if (!userId || !billingAddress || !shippingAddress) {
      return NextResponse.json({ error: "User ID, billingAddress, and shippingAddress are required" }, { status: 400 })
    }
    ...
    if (existingAddress) {
      ...
      return NextResponse.json({ message: "Addresses updated successfully", data: existingAddress }, { status: 200 })
    } else {
      ...
      return NextResponse.json({ message: "Addresses created successfully", data: newAddress }, { status: 201 })
    }
  } catch (error) {
    console.error("Error saving addresses:", error)
    return NextResponse.json({ error: "Error saving addresses" }, { status: 500 })
  }
}
